 ...
 * 
 * !! ATTENTION : Fichier modifié par Netactions par rapport �  la version originale
 */
...
;(function($) {
		...
		var $element, conHeight, scrollPaneApi, slideInt, titles = [], dates = [], summaries = [], descriptions = [], links = [], previewCons = [], topConIndex = 0, busy = true, isSliding = false, slideDirection = "next";
			...
			function _loadFromDom(domList){
				...
				for(var i=0; i < domList.length; ++i){
					...
					// Modif netactions : on peut préciser le lien de l'actu dans "data-link"
					links[i] = $(domList[i]).data('link');
					...
					previews[i] = "<div class='fn-newsPreview' style='width:"+options.width+"px; height:"+conHeight+"px; background:"+options.backgroundColor+";background-color: rgba(0, 0, 0, 0.7);'>"+imageDom+"<span class='fn-newsPreviewText' ><h3>"+titles[i]+"</h3><h4>"+dates[i]+"</h4>"+summaries[i]+"</span></div>";
				};
			...
			function _createPreviews(previews){
				...
				.click(function(){
					...
					// Modif Netactions : on active le clic même sur les actu dans le DOM
	...
	function _footerText(top, bottom, length){
		// Modif Netactions : on n'affiche rien dans le footer
		// var text = String(top+'-'+bottom+' sur '+length+' actualites');
		// return text;
		return '';
	};
	
	$.fn.fancyNews.defaults = {
		width: 400, //the width of the main content area in px(without footer bar)
		height: 240, //the height of the main content area in px(without footer bar)
		previewOffset: 2, //the offset between each preview in px
		maxWords: 50, //the numbers of words for each preview text
		previewsPerPage: 2, //the numbers of previews per page
		...
		useLinks: true, //enable or disable each preview as referer to the origin blog entry (only for RSS feed)
		targetWindow: '_self'
	};
})(jQuery);
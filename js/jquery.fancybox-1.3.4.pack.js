		...
		if (f.is(":visible") && false === d.onCleanup(l, p, d)) {
			...
			if (d.titleShow !== false) {
				if (b.isFunction(d.titleFormat))
					...
					a = s && s.length ? d.titlePosition == "float" ? '<table id="fancybox-title-float-wrap" cellpadding="0" cellspacing="0"><tr><td id="fancybox-title-float-left"></td><td id="fancybox-title-float-main">'
							+ s
			...
			if (f.is(":visible")) {
				...
				if (d.transitionIn == "elastic") {
					...
					f
							.css(i)
							.fadeIn(d.transitionIn == "none" ? 0 : d.speedIn, S)
				}
			}
		}
	}, Y = function() {

...
$content_frames = array("lz_chat_frame.3.2.login.1.0","lz_chat_frame.3.2.mail.1.0","lz_chat_frame.3.2.chat.1.0","lz_chat_frame.3.2.chat.0.0","lz_chat_frame.3.2.chat.2.0");
...
if(!empty($USER->Browsers[0]->DesiredChatGroup) && ($conline || empty($_GET["ovloo"])) && !IS_FILTERED)
{
	$changed=false;
	...
	{
		$USER->Browsers[0]->UpdateArchive($USER->Browsers[0]->Email,$USER->Browsers[0]->Email,((!empty($USER->Browsers[0]->Fullname)) ? $USER->Browsers[0]->Fullname : $LZLANG["client_guest"]));
		$chat_available = true;
	...
	$USER->AddFunctionCall("lz_chat_set_application(".parseBool($chat_available).",".parseBool($BOTMODE).",".($OPERATOR_COUNT).");",false);
	...
	while(!empty($_GET["mi".$pc]))
	{
		...
		if(!$USER->Browsers[0]->Waiting)
		{
			...
			if(!empty($USER->Browsers[0]->InternalUser) && $USER->Browsers[0]->InternalUser->IsBot)
			{
				$rpost = new Post($id = getId(32),$USER->Browsers[0]->InternalUser->SystemId,$USER->Browsers[0]->SystemId,$answer=$USER->Browsers[0]->InternalUser->GetBotAnswer($post->Text,true,$USER->Browsers[0]),time(),$USER->Browsers[0]->ChatId,$INTERNAL[$systemid]->Fullname);
	...
	else if($chat_available && ((empty($USER->Browsers[0]->Forward) && !(!empty($USER->Browsers[1]->ChatRequest) && !$USER->Browsers[1]->ChatRequest->Closed) && empty($USER->Browsers[0]->InternalUser) && !$USER->Browsers[0]->Waiting) || (!empty($USER->Browsers[0]->InternalUser) && $USER->Browsers[0]->InternalUser->IsBot && $LMRFLAG=="ONM01") || $FULL))
	{
		...
		else if($BOTMODE && (($LMRFLAG!="OBM01" || $FULL) && ( (empty($USER->Browsers[0]->InternalUser) && empty($_GET["op"])) || (!empty($USER->Browsers[0]->InternalUser) && $USER->Browsers[0]->InternalUser->IsBot))))
		{
			...
			if(!empty($INTERNAL[$USER->Browsers[0]->DesiredChatPartner]) && $INTERNAL[$USER->Browsers[0]->DesiredChatPartner]->IsBot)
			{
				...
				$image = "<img src=\"".LIVEZILLA_URL . $INTERNAL[$USER->Browsers[0]->DesiredChatPartner]->GetOperatorPictureFile()."\" width=\"52\" height=\"39\">";
	...
	if(!empty($_GET["tid"]))
	{
		...
		{
			$USER->Browsers[0]->SaveLoginData();
			$USER->SendCopyOfMail($ticket,"",false);
		}
	}
	...
	$USER->AddFunctionCall("lz_chat_set_connecting(".parseBool(!$BOTMODE && (!empty($USER->Browsers[0]->ChatId) && !$USER->Browsers[0]->InternalActivation && !$USER->Browsers[0]->Closed && !$USER->Browsers[0]->Declined)).",'".$USER->Browsers[0]->SystemId."');",false);
...
function speakingToHTML()
{
	...
	if(!empty($USER->Browsers[0]->InternalUser))
	{
		...
		if(!$USER->Browsers[0]->Closed && $USER->Browsers[0]->InternalActivation && empty($_GET["op"]))
		{
			...
			$image = "<img align=\"left\" src=\"".LIVEZILLA_URL . $USER->Browsers[0]->InternalUser->GetOperatorPictureFile()."\" width=\"52\" height=\"39\">";
...
{
	global $USER,$INTERNAL,$LANGUAGE;
	$html = getFile(TEMPLATE_HTML_MESSAGE_OVERLAY_CHAT_INVITE);
	$html = str_replace("<!--display_image-->","''",$html);
	$html = str_replace("<!--image-->","<img align=\"left\" src=\"".LIVEZILLA_URL.$INTERNAL[$_operatorID]->GetOperatorPictureFile()."\" width=\"52\" height=\"39\">",$html);
...
function initChat()
{
	...
	if($USER->Browsers[0]->Status == CHAT_STATUS_OPEN)
	{
		...
		if((count($INTLIST) + $INTBUSY) > 0)
		{
			...
			else if(!empty($CONFIG["gl_mqwt"]) && (time()-$USER->Browsers[0]->FirstActive) > ($CONFIG["gl_mqwt"]*60))
			{

...
{
	initData(false,false,DB_ACCESS_REQUIRED);
...
if(defined("VALIDATED") && !SERVERSETUP)
{
	if(LOGOFF || LOGIN)
	{
		...
		//queryDB(false,"UPDATE `".DB_PREFIX.DATABASE_VISITOR_CHAT_OPERATORS."` SET `status`=9,`ltime`=".time()." WHERE `user_id`='".@mysql_real_escape_string(CALLER_SYSTEM_ID)."' AND `status` != 9 AND `status` != 2;");

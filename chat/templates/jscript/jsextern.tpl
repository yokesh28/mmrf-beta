...
function lz_chat_reload_groups()
{
	...
	{
		lz_chat_change_group(lz_chat_get_frame_object('lz_chat_frame.3.2.lgin.1.0','lz_form_groups'));
...
{	
	...
	if(_errors.length > 0)
	{
		...
		lz_chat_chat_alert(_errors,lz_chat_get_frame_object('lz_chat_frame.3.2.lgin.1.0',''),-1);
		lz_chat_set_button_text(null,false);
	}
	else
		setTimeout("lz_chat_reload_groups();",120000);
}
...
function lz_ticket_change_group(_box)
{
	...
	{
		lz_chat_set_button_text(lz_chat_data.SelectedGroup,true);
		lz_chat_set_input_fields(lz_chat_data.SelectedGroup,lz_chat_get_frame_object('lz_chat_frame.3.2.mail.1.0','').document);
		return;
	}
}
...
function lz_chat_change_group(_box)
{
	...
	if(lz_chat_data.SelectedGroup != null)
	{
		...
		lz_chat_set_button_text(lz_chat_data.SelectedGroup,false);
		lz_chat_set_input_fields(lz_chat_data.SelectedGroup,lz_chat_get_frame_object('lz_chat_frame.3.2.lgin.1.0','').document);
	}
}

function lz_chat_set_button_text(_selGroup,_onlineOnly)
{
	var infoFieldText = lz_chat_data.Language.InfoFieldText;
	var online = ((_selGroup != null && lz_chat_data.SelectedGroup.Amount > 0) || lz_chat_data.NoPreChatMessages);

	if(_selGroup != null && online && _selGroup.CallMeBackInformation.length>0 && lz_chat_data.CallMeBackMode)
		infoFieldText += (infoFieldText.length > 0) ? ("<br>" + _selGroup.CallMeBackInformation) : _selGroup.CallMeBackInformation;
	else if(_selGroup != null && online && !_onlineOnly && _selGroup.ChatInformation.length>0)
		infoFieldText += (infoFieldText.length > 0) ? ("<br>" + _selGroup.ChatInformation) : _selGroup.ChatInformation;
	else if((!online || lz_chat_data.CurrentApplication!="chat") && _selGroup != null && _selGroup.TicketInformation.length>0)
		infoFieldText += (infoFieldText.length > 0) ? ("<br>" + _selGroup.TicketInformation) : _selGroup.TicketInformation;

	if(lz_chat_data.CurrentApplication=="chat")
	{
		lz_chat_get_frame_object('lz_chat_frame.3.2.lgin.1.0','buy_voucher_button').style.display = (_selGroup.ChatVouchersRequired.length > 0 && online) ? '' : 'none';
		lz_chat_get_frame_object('lz_chat_frame.3.2.lgin.1.0','lz_action_button').value = (online) ? ((lz_chat_data.CallMeBackMode) ? lz_chat_data.Language.ClientCallMeNow : lz_chat_data.Language.StartChat) : lz_chat_data.Language.SendMessage;
		lz_chat_get_frame_object('lz_chat_frame.3.2.lgin.1.0','lz_input_header_title').innerHTML = "<b>" + ((online) ? ((lz_chat_data.CallMeBackMode) ? lz_chat_data.Language.ClientRequestInstantCallback : lz_chat_data.Language.StartChat) : lz_chat_data.Language.LanguageLeaveMessage) + "</b>";
		lz_chat_get_frame_object('lz_chat_frame.3.2.lgin.1.0','lz_form_info_field').innerHTML = (online) ? ((infoFieldText.length > 0) ? infoFieldText : ((lz_chat_data.CallMeBackMode) ? lz_chat_data.Language.ClientRequestInstantCallbackInfo : ((lz_chat_data.SelectedGroup.ChatVouchersRequired.length > 0) ? lz_chat_data.Language.StartChatComInformation : lz_chat_data.Language.StartChatInformation))) : (infoFieldText.length > 0) ? infoFieldText : lz_chat_data.Language.LanguageLeaveMessageInformation;
	}
	else
		lz_chat_get_frame_object('lz_chat_frame.3.2.mail.1.0','lz_form_info_field').innerHTML = (online && !_onlineOnly) ? ((infoFieldText.length > 0) ? infoFieldText : lz_chat_data.Language.StartChatInformation) : (infoFieldText.length > 0) ? infoFieldText : lz_chat_data.Language.LanguageLeaveMessageInformation;
}
...
function lz_chat_set_input_fields(_selGroup,_document)
{
	if(lz_chat_data.InputFieldIndices != null)
	{
		...
		var mandatoryFields = (lz_chat_data.CurrentApplication=="chat" && _selGroup.ChatVouchersRequired.length > 0);
...
function lz_chat_validate_group()
{
	...
	if(lz_chat_data.SelectedGroup == null)
	{
		lz_chat_chat_alert(lz_chat_data.Language.SelectValidGroup,lz_chat_get_frame_object('lz_chat_frame.3.2.lgin.1.0',''));
		return false;	
	}
...
function lz_chat_goto_message(_inChat,_send)
{
	...
	{
		lz_chat_chat_alert(lz_chat_data.Language.ClientNoInternUsersShort,lz_chat_get_frame_object('lz_chat_frame.3.2.lgin.1.0',''));
		return;
	}
	...
	{
		var email = (lz_chat_data.SelectedGroup != null) ? lz_chat_data.SelectedGroup.Email : lz_chat_data.Groups.GetGroupById(lz_chat_get_frame_object('lz_chat_frame.3.2.lgin.1.0','lz_form_groups').value).Email;
	...
	lz_chat_set_status(lz_chat_data.STATUS_STOPPED);
	lz_chat_data.PermittedFrames = 5;
	lz_chat_data.CurrentApplication = "ticket";
	lz_chat_data.DirectSendTicket = _send;
...
function lz_load_input_values(_frame)
{
	...
	if(_frame == 'lz_chat_frame.3.2.mail.1.0' && lz_chat_data.SelectedGroup != null)
	{
		var box = lz_chat_get_frame_object('lz_chat_frame.3.2.mail.1.0','lz_form_groups');
		for(var i=0;i<box.options.length;i++)
			if(box.options[i].value == lz_chat_data.SelectedGroup.Id)
			{
				box.selectedIndex = i;
				lz_ticket_change_group(box);
				return;
			}
	}
	
	if(lz_chat_data.DirectLogin && (!lz_chat_validate_group() || !lz_check_missing_inputs(lz_chat_get_frame_object('lz_chat_frame.3.2.lgin.1.0',''),true,'lz_chat_check_login_inputs','lz_chat_frame.3.2.lgin.1.0')))
		lz_chat_data.DirectLogin = false;
}

function lz_release_ticket_form()
{
	lz_load_input_values('lz_chat_frame.3.2.mail.1.0');
	if(lz_chat_data.DirectSendTicket)
		lz_chat_check_ticket_inputs();
	else
	{
		lz_chat_get_frame_object('lz_chat_frame.3.2.mail.1.0','lz_chat_loading').style.display='none';
		lz_chat_get_frame_object('lz_chat_frame.3.2.mail.1.0','lz_form_details').style.display='';
	}
	lz_chat_announce_to_parent();
}
...
function lz_chat_release(_groupAvailable, _groupError)
{
	...
	{
		lz_chat_chat_alert(_groupError,lz_chat_get_frame_object('lz_chat_frame.3.2.lgin.1.0',''),-1);
		lz_chat_get_frame_object('lz_chat_frame.3.2.lgin.1.0','').document.body.removeChild(lz_chat_get_frame_object('lz_chat_frame.3.2.lgin.1.0','lz_chat_loading'));
		return;
	}
	else
	{
...
function lz_check_missing_inputs(_frame,_display,_contFunc,_frameName)
{
	...
	for(var i = 0;i < lz_chat_data.InputFieldIndices.length;i++)
	{
		...
		{
			if((((findex == 115 && lz_chat_data.SelectedGroup.ChatVouchersRequired.length > 0 && lz_chat_data.CurrentApplication=="chat") || (lz_array_indexOf(ihidden,findex) == -1 && (lz_array_indexOf(imandatory,findex) != -1 || lz_chat_data.InputFieldValues[i].Validation))) || (findex == 116 && lz_chat_data.CallMeBackMode)) && lz_global_trim(_frame.document.getElementsByName("form_" + findex)[0].value).length == 0)
			{
				if(_display)
...
function lz_validate_inputs(_contFunc,_frame,_hidden)
{
	...
	for(var i = 0;i < lz_chat_data.InputFieldValues.length;i++)
	{
		...
		else if(lz_chat_data.CurrentApplication=="chat" && lz_chat_data.ComChatInput == null && lz_chat_data.InputFieldValues[i].Index == 115 && lz_chat_data.SelectedGroup.ChatVouchersRequired.length > 0)
		{
			...
			lz_chat_data.ComChatInput = new lz_chat_input(115,true,einput.Caption,einput.Name,einput.Type,lz_global_base64_encode(einput.Value),true,lz_global_base64_encode("<!--server-->validate.php?value=<!--value--><!--website-->&intgroup=" + lz_chat_data.SelectedGroup.Id),5,false);
	...
	for(var i = 0;i < lz_chat_data.InputFieldValues.length;i++)
	{
		lz_chat_data.InputFieldValues[i].SetStatus(_frame,true);
	}
	return true;
}
...
function lz_validate_com_chat_input_result(_result,_inuse,_reason,_voucherid,_chatTime,_chatTimeMax,_chatSessions,_chatSessionsMax,_expires,_expired)
{
	if(!_result)
	{
		...
		if(_inuse)
			lz_chat_chat_alert(lz_chat_data.Language.ClientVoucherInUse,lz_chat_get_frame_object('lz_chat_frame.3.2.lgin.1.0',''),null);
		else if(_reason==0)
			lz_chat_chat_alert(lz_chat_data.Language.ClientInvalidComChatAccount,lz_chat_get_frame_object('lz_chat_frame.3.2.lgin.1.0',''),null);
		else
			lz_chat_chat_alert(lz_chat_data.Language.ClientEmptyComChatAccount,lz_chat_get_frame_object('lz_chat_frame.3.2.lgin.1.0',''),null);
	}
	else
	{
...
function lz_validate_input_result(_result,_id)
{
	for(var i = 0;i < lz_chat_data.InputFieldValues.length;i++)
	{
		if(lz_chat_data.CurrentApplication=="chat")
			...
			lz_chat_data.InputFieldValues[i].SetStatus("lz_chat_frame.3.2.mail.1.0",true);
	}
	...
	for(var i = 0;i < lz_chat_data.InputFieldValues.length;i++)
	{
		...
		if(failed)
		{
			...
			if(cinput != lz_chat_data.ComChatInput)
				lz_chat_chat_alert(lz_chat_data.Language.ClientInvalidData + " " + cinput.Caption.replace(":",""),lz_chat_get_frame_object(cinput.ValidationFrame,''),null);
...
{
	if(!lz_check_missing_inputs(lz_chat_get_frame_object('lz_chat_frame.3.2.mail.1.0',''),true,'lz_chat_check_ticket_inputs','lz_chat_frame.3.2.mail.1.0'))
		return;
	if(lz_chat_data.InputFieldValues[lz_array_indexOf(lz_chat_data.InputFieldIndices,111)].Active)
		lz_chat_data.ExternalUser.Username = lz_chat_get_frame_object('lz_chat_frame.3.2.mail.1.0','').document.getElementsByName("form_111")[0].value.substr(0,255);
	if(lz_chat_data.InputFieldValues[lz_array_indexOf(lz_chat_data.InputFieldIndices,112)].Active)
		lz_chat_data.ExternalUser.Email = lz_chat_get_frame_object('lz_chat_frame.3.2.mail.1.0','').document.getElementsByName("form_112")[0].value.substr(0,255);
	if(lz_chat_data.InputFieldValues[lz_array_indexOf(lz_chat_data.InputFieldIndices,113)].Active)
		lz_chat_data.ExternalUser.Company = lz_chat_get_frame_object('lz_chat_frame.3.2.mail.1.0','').document.getElementsByName("form_113")[0].value.substr(0,255);
	if(lz_chat_data.InputFieldValues[lz_array_indexOf(lz_chat_data.InputFieldIndices,114)].Active)
		lz_chat_data.ExternalUser.MailText = lz_chat_get_frame_object('lz_chat_frame.3.2.mail.1.0','').document.getElementsByName("form_114")[0].value.substr(0,16777216);
	if(lz_chat_data.InputFieldValues[lz_array_indexOf(lz_chat_data.InputFieldIndices,116)].Active || lz_chat_data.CallMeBackMode)
		lz_chat_data.ExternalUser.Phone = lz_chat_get_frame_object('lz_chat_frame.3.2.mail.1.0','').document.getElementsByName('form_116')[0].value.substr(0,255);
	
	lz_chat_data.ExternalUser.Group = lz_chat_get_frame_object('lz_chat_frame.3.2.mail.1.0','lz_form_groups').value;
	lz_chat_send_mail();
}

function lz_chat_send_mail()
{
	lz_chat_get_frame_object('lz_chat_frame.3.2.mail.1.0','lz_action_button').disabled=true;
	lz_chat_get_frame_object('lz_chat_frame.3.2.mail.1.0','lz_action_button').style.cursor='wait';
...
{
	lz_chat_get_frame_object('lz_chat_frame.3.2.mail.1.0','lz_action_button').style.cursor='pointer';
	lz_chat_get_frame_object('lz_chat_frame.3.2.mail.1.0','').document.getElementById('lz_chat_loading').style.display='none';
	lz_chat_get_frame_object('lz_chat_frame.3.2.mail.1.0','').document.getElementById('lz_form_details').style.display='';
	if(_success)
	{
		lz_chat_get_frame_object('lz_chat_frame.3.2.mail.1.0','lz_chat_ticket_success').style.display='';
		lz_chat_get_frame_object('lz_chat_frame.3.2.mail.1.0','lz_chat_ticket_details').style.display='none';
		lz_chat_get_frame_object('lz_chat_frame.3.2.mail.1.0','lz_chat_ticket_header').style.display='none';
	}
	else
	{
		lz_chat_get_frame_object('lz_chat_frame.3.2.mail.1.0','lz_action_button').disabled = false;
		lz_chat_chat_alert(lz_chat_data.Language.MessageFlood,lz_chat_get_frame_object('lz_chat_frame.3.2.mail.1.0',''));
	}
}
...
function lz_chat_close()
{
	...
	if(lz_chat_data.GetParameters.indexOf("mp") == -1)
		lz_chat_change_url("./<!--file_chat-->?"+lz_chat_data.GetParameters + "&mp=1");
	else
...
function lz_chat_file_request_upload()
{
	...
	if(!lz_chat_data.FileUpload.Running)
	{
		...
		{
			lz_chat_chat_alert(lz_chat_data.Language.SelectFile,lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0',''),null);
			return;
		}
...
function lz_chat_file_error(_value)
{
	...
	if(_value == lz_chat_data.FILE_UPLOAD_REJECTED)
		lz_chat_chat_alert(lz_chat_data.Language.FileUploadRejected,lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0',''),null);
	else if(_value == lz_chat_data.FILE_UPLOAD_OVERSIZED)
		lz_chat_chat_alert(lz_chat_data.Language.FileUploadOversized,lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0',''),null);
...
function lz_chat_file_ready()
{
	...
	lz_chat_chat_alert(lz_chat_data.Language.FileProvided,lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0',''),null);
}
...
function lz_global_replace_smilies(_text)
{
	...
	for(var i = 0;i<shorts.length;i++)
		_text = _text.replace(shorts[i]," <img border=0 src='./images/smilies/"+images[i]+".png'> ");
	return _text;
}
  ...
  <head>
	...
	*{font-family:verdana,arial;font-size:11px;color:#616161;}
	...
	function DoZoomTo(_to,_selected)
	{
		...
		document.getElementById("map").style.backgroundImage="url('"+path+maps[_to].file+"')";
		document.getElementById("map").style.width=maps[_to].width;
		document.getElementById("map").style.height=maps[_to].height;
		...
		if(level > _to)
			...
			map_scroll.ScrollTo(map_scroll.map.style.left.replace("px","")-((maps[_to].width-maps[level].width)/2),map_scroll.map.style.top.replace("px","")-((maps[_to].height-maps[level].height)/2));

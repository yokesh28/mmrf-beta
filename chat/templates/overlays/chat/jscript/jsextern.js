...
function lz_chat_set_queued_posts(_html)
{
	var qpe = document.getElementById("lz_chat_queued_posts");
	if(qpe == null)
	{
		var qpe = document.createElement("div");
		qpe.id = "lz_chat_queued_posts";
		document.getElementById("lz_chat_content_box").appendChild(qpe);
	}
	document.getElementById("lz_chat_queued_posts").innerHTML = lz_global_base64_url_decode(_html);
	lz_chat_scoll_down();
...
function lz_chat_set_focus(_chat)
{
	...
	{
		if(!_chat)
			setTimeout("document.getElementById('lz_chat_overlay_ticket_message').focus();",50);
		else
			setTimeout("document.getElementById('lz_chat_text').focus();",50);
	}
}
...
function lz_chat_set_connecting(_connecting,_id)
{
	if(_id != null)
		lz_external.Id = _id;
...
function lz_chat_message_return()
{
	...
	lz_chat_set_application(true);
}

function lz_chat_set_application(_chat)
{
	...
	if(lz_tickets_external && !_chat && lz_chat_state_expanded)
		lz_chat_change_state(true,true);
	...
	if(lz_chat_application != _chat)
	{
		...
		if(!_chat && lz_last_post.length > 0 && document.getElementById('lz_chat_overlay_ticket_message').value.length == 0)

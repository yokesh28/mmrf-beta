...
function buildEvents()
{
	...
	if(!empty($EVENTS))
		...
		{
			if(DEMO_MODE && $event->Creator != CALLER_SYSTEM_ID)
				continue;
...
function buildChatVouchers($xml="",$last=0,$value="",$typecond="")
{
...
function buildResources($xml="",$count=0,$last=0)
{
	...
	$demores = (DEMO_MODE) ? " AND `editor`='".@mysql_real_escape_string($_SERVER["REMOTE_ADDR"])."'" : "";

	if($result = queryDB(true,"SELECT * FROM `".DB_PREFIX.DATABASE_RESOURCES."` WHERE `edited` > ".@mysql_real_escape_string($_POST[POST_INTERN_XMLCLIP_RESSOURCES_END_TIME])." AND `edited`<".@mysql_real_escape_string(time()).$demores." ORDER BY `edited` ASC"))
	{
		while($row = mysql_fetch_array($result, MYSQL_BOTH))
			$resources[] = $row;
	}
...
{
	global $RESPONSE,$INTERNAL,$GROUPS,$INPUTS;
	initData(false,false,false,false,false,false,false,true);
					...
					while($rowc = mysql_fetch_array($resultc, MYSQL_BOTH))
						foreach($INPUTS as $index => $input)
							if($input->Name == $rowc["custom_id"] && $input->Active)
								$ticket->Customs[$rowc["custom_id"]] = $input->GetClientValue($rowc["value"]);
			}

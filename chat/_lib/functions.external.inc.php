...
function getPredefinedMessage($_list, $_user)
{
	...
	{
		if(($message->IsDefault && (!$message->BrowserIdentification || empty($_user->Language))) || ($message->BrowserIdentification && !empty($_user) && $_user->Language == $message->LangISO))
		{
			$sel_message = $message;
			$break = true;
			break;
...
{
	global $CONFIG,$GROUPS,$INTERNAL,$USER,$VOUCHER;
	$start = time();
...
function getNewPosts()
{
	...
	{
		$senderName = (!empty($post->SenderName)) ? $post->SenderName : $USER->UserId;
...
function getInternal($desired="",$util=0,$fromCookie=null,$result=true,$_allowBots=false,$_requireBot=false)
{
	...
	{
		if($internal->LastActive > (time()-$CONFIG["timeout_clients"]) && ($_allowBots || !$internal->IsBot) && (!$_requireBot || $internal->IsBot))
		{
